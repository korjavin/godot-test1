...
| 4 | **Coin settling** — `_settle_coin_y`, `COIN_GROUND_HEIGHT`, **and `Coin.id_at`** | **medium** | `prop_selfcheck`, `enemy_spawn_selfcheck` check 14, **`mp_selfcheck`** | perches on a climbable top, which is 3's output; coins are the headline score, so this is the first one a player feels. **The multiplayer half is the trap**: `Coin.id_at()` hashes `roundi(pos.y * COIN_ID_QUANT)` (8 cells per metre) and that id is the pickup key for `claim_pickup` and the join snapshot. Coin Y is a constant today outside a perch; after this row it is `height_at()`, which by its own docstring is **not** peer-deterministic (the corridor window is centred on the LOCAL player, and a focus-pinned chunk bakes off a window that may hold no corridor). Two peers then hold different ids for one field coin — it is never despawned and the join replay misses it. Road coins are safe because the corridor is forced flat, so this appears only on the field spawners, which is exactly the shape that survives a smoke test. **Prerequisite**: either thread an explicit `(k, slot)` / `(chunk, index)` id out of the three spawners (the upgrade path `Coin.id_at`'s own `ponytail:` comment already names) or make `height_at` peer-deterministic, which is the work row 2 defers |
| 5 | **Crocodile / boss gravity settle**, **plus the two 3-D distance tests** | **medium** | `enemy_spawn_selfcheck`, `boss_selfcheck` | species settle to flat ground. The boss territory leash really is XZ and stays XZ (`in_territory()` builds a `Vector2`), but **the other two are not, and the first draft of this row said they were**: `crocodile_lod_manager.gd`'s sleep/vignette scan compares full `Vector3`s (`croc_pos.distance_squared_to(player_pos)` / `…(focus_points[i])`), and `_update_chase_state`'s `distance_to_player` is a 3-D `global_position.distance_to(...)` measured against `detection_radius`. Both become height-dependent the instant the field is displaced — at the mountain amplitude a predator 12 m away horizontally but 14 m up a slope is 18.4 m in 3-D against a wolf's `detection_radius` 18 and never acquires, and the `SIM_RADIUS` 45 sleep boundary moves the same way. Flattening both to XZ is the same one-line change `in_territory()` already makes and leaves CLAUDE.md's `SIM_RADIUS ≫ detection_radius` chain intact. The two named checks assert today's behaviour against code that still writes y = 0, so **they cannot see this** — it needs an explicit leg |
...
- **Projectiles, weather, the hunt director.** All XZ or transient; none reads ground
  height. **The LOD manager is NOT on this list** — it reads no ground height either,
  but its radii are 3-D `distance_squared_to` on full `Vector3`s, so displacement moves
  the sleep boundary. It is consumer 5, not a non-consumer.
