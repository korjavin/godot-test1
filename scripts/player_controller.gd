...
## THE FOUR F ABILITIES, in `scripts/player_abilities.gd` (bd godot-test1-ftn.15).
## Same seam and same reason as `anim` above, one concern along — except that the
## ability STATE VARS stay on this node (see the file's header): four self-checks,
## `mp_manager`'s `ab` presence bits, `player_animation` and every HUD getter below
## bind them here, so the arms write them through their `player` back-reference.
...
func _init() -> void:
	"""
	Hand the animation and ability helpers their back-reference, before ANY tree
	callback runs.

	`_ready()` would be too late for a harness that pokes the pose on a freshly
	instanced body, and it is the only wiring either seam needs.
	"""
	anim.player = self
...
const TEIBI_FORM_DURATION := PlayerAbilities.TEIBI_FORM_DURATION
const TEIBI_REVERT_RETRY := PlayerAbilities.TEIBI_REVERT_RETRY
const TEIBI_QUAKE_FLEE_DURATION := PlayerAbilities.TEIBI_QUAKE_FLEE_DURATION
const PHOBOMAN_FLEE_DURATION := PlayerAbilities.PHOBOMAN_FLEE_DURATION
const PHOBOMAN_STINK_RADIUS := PlayerAbilities.PHOBOMAN_STINK_RADIUS
const PHOBOMAN_FLEE_RADIUS := PlayerAbilities.PHOBOMAN_FLEE_RADIUS
# ----------------------------------------------------------------------------
...
##
## THE ONE ABILITY CONSTANT THAT DID NOT MOVE (bd godot-test1-ftn.15): it is
## derived from `WALK_SPEED`, which is this file's, and `PlayerAbilities` cannot
## `preload` this script back without the cyclic dependency `player_animation.gd`
## already documents. Restating `5.0 *` over there would let a walk-speed retune
## stop reaching the Air Rush, so the const stays with the number it derives from
...
# ============================================================================
# THE ABILITY SURFACE — one forwarder per arm, and every one is load-bearing
# ============================================================================
## The bodies live in `scripts/player_abilities.gd` (bd godot-test1-ftn.15); what
## stays here is the NAME, because the "player" group answers to one node and
## these names are what everything else binds to: `ability_hud.gd` and the touch
## controls press `try_activate_ability`, `mp_manager.gd` relays a room-mate's
## Stink Wave through `_ability_phoboman`, `tower_interior.gd` measures a blink
## with `_ability_primm`, `player_animation.gd` reads `_current_teibi_scale` and
## spawns its dust through `_spawn_ability_effect`, `coin.gd` pops the same
## effect, and six self-checks drive the rest through `player.call("...")`.
## That is the epic's rule-(d) exception, named in the bead.
##
## They are also why NOTHING ELSE in this file moved: every internal call site
## above and below still calls the same name on `self`.
