...
func _check_scarcity(terrain_script: GDScript, consts: Dictionary) -> void:
	...
	# Far field beyond plain (east) builds nothing; near field (inside union) builds non-empty.
	var far_pos := Vector3(east_edge + plain + 200.0, 0.0, 0.0) # ~200 m beyond plain distance
	var far_chunk: Vector2i = terrain.world_to_chunk(far_pos)
	# Find a far chunk whose biome actually builds content (not PLAINS) so biome check is not vacuous.
	var far_biome_chunk := far_chunk
	var found_biome := false
	for r in range(5):
		for dx in range(-r, r+1):
			for dz in range(-r, r+1):
				var c2: Vector2i = far_chunk + Vector2i(dx, dz)
				var centre2: Vector3 = terrain.chunk_to_world(c2)
				if terrain.biome_at(centre2.x, centre2.z) != 0: # 0 == PLAINS
					far_biome_chunk = c2
					found_biome = true
					break
			if found_biome:
		...
		if found_biome:
			break
	# Build a 5x5 field at far distance and count.
	var far_counts := {"props": 0, "camps": 0, "artifacts": 0, "landmarks": 0, "chests": 0, "biome": 0}
	for dx in range(-2, 3):
		for dz in range(-2, 3):
			var cp: Vector2i = far_biome_chunk + Vector2i(dx, dz)
			...
			var body := StaticBody3D.new()
			var plats: Array = []
			terrain.spawn_objects_in_chunk(cp, plats, batch, body)
	...
	for k: String in far_counts.keys():
		if int(far_counts[k]) != 0:
			_fail("scarcity: far field beyond plain builds %d %s — plain terrain at ≥4 km must be empty" % [int(far_counts[k]), k])

	# Near-field control: same sized field inside the union (corridor) must be non-empty in every category.
	var near_pos := Vector3(0.0, 0.0, 0.0) # origin is inside corridor (x between -465 and 1600, z 0)
	# If origin's biome is PLAINS, shift to a nearby forest chunk for biome control.
	var near_chunk: Vector2i = terrain.world_to_chunk(near_pos)
	var near_biome_chunk := near_chunk
	var near_found := false
	for r in range(10):
		for dx in range(-r, r+1):
			for dz in range(-r, r+1):
				var c2: Vector2i = near_chunk + Vector2i(dx, dz)
				var centre2: Vector3 = terrain.chunk_to_world(c2)
				# Must be inside union (k~1) and biome not PLAINS.
				var k2 := float(terrain.call("scarcity_at", centre2))
				if k2 > 0.99 and terrain.biome_at(centre2.x, centre2.z) != 0:
					near_biome_chunk = c2
					near_found = true
					break
			if near_found:
		...
		if near_found:
			break
	var near_counts := {"props": 0, "camps": 0, "artifacts": 0, "landmarks": 0, "chests": 0, "biome": 0}
	for dx in range(-2, 3):
		for dz in range(-2, 3):
			var cp: Vector2i = near_biome_chunk + Vector2i(dx, dz)
			var batch: Array = []
			var body := StaticBody3D.new()
			var plats: Array = []
			terrain.spawn_objects_in_chunk(cp, plats, batch, body)
			near_counts["props"] += batch.size()
			body.free()
			var camp: Dictionary = terrain.call("_camp_at", cp)
			if not camp.is_empty():
				near_counts["camps"] += 1
			var art: Dictionary = terrain.call("_artifact_at", cp)
			if not art.is_empty():
				near_counts["artifacts"] += 1
			var lm: Dictionary = terrain.call("_landmark_at", cp)
			if not lm.is_empty():
				near_counts["landmarks"] += 1
			var ch: Dictionary = terrain.call("_chest_at", cp)
			if not ch.is_empty():
				near_counts["chests"] += 1
			var b_batch: Array = []
			var b_body := StaticBody3D.new()
			var b_obs: Array = []
			terrain.call("spawn_biome_content_in_chunk", cp, b_obs, b_batch, b_body)
			near_counts["biome"] += b_batch.size()
			b_body.free()
	for k: String in near_counts.keys():
		if int(near_counts[k]) == 0:
			_fail("scarcity: near field inside union builds 0 %s — control must be non-empty, or far empty proves nothing" % k)
