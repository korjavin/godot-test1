...
##  17. THE WEB RESIDENCY WINDOW, folded into check 4. Filling the blocks (bead
##      godot-test1-8gw.9) moved the city's cost from "one expensive chunk" to
##      "1,631 ordinary ones", and a per-chunk ceiling cannot see that at all.
##  18. DETERMINISM. Every city chunk byte-identical across two run seeds, and
##      the seeded streams outside the rect A/B'd against a city-disabled build.
##
## ==================== WHAT IS IN THE OTHER FILE ====================
##
## `scripts/budapest_city_selfcheck.gd` carries the checks about the city the
## chunks BUILD — 11 (the plateau ramps), 13 (the gate avenue is walkable), 14
## (the four bridges, Margaret Island, the Danube's crocodiles, and nothing
## standing in the river), 15 (the city is full, and still a city) and 16
## (reachability). Bead `godot-test1-ftn.13` split them out: CI's
## `selfcheck-shard` globs `scripts/*_selfcheck.gd` and shards BY FILE, so a
## 4,100-line check that is a third of the suite's wall clock is a shard nothing
## can balance. The split is mechanical and the NUMBERING IS UNCHANGED on both
## sides — a check number is quoted in CLAUDE.md, in the bead trail and in the
## failure messages, so the file it lives in is not what names it.
##
## Deliberately NOT covered here or there: how any of it LOOKS. These files
## measure the contracts; the eye measures the rest.
##
...
func _run() -> void:
	...
	# CHECKS 11, 13, 14, 15 AND 16 ARE NOT HERE. The ramps, the avenue, the four
	# bridges, the filled blocks and the reachability flood live in
	# `scripts/budapest_city_selfcheck.gd`, split out by bead `godot-test1-ftn.13`
	# because CI shards `scripts/*_selfcheck.gd` BY FILE and this one was a third of
	# the suite's wall clock on its own. The numbering is unchanged on both sides:
	# a check number is quoted in CLAUDE.md, in the bead trail and in the failure
	# messages, so the file it lives in is not what names it.
...
# ============================================================================
# CHECK 18 — DETERMINISM: every city chunk identical, outside streams unchanged
# ============================================================================

func _check_determinism_every_chunk(terrain_script: GDScript) -> void:
	"""
	Check 18. Every city chunk regenerates byte-identical across the two run
	seeds (the city is authored, no draw may leak), and the crocodile/hunter
	streams outside the rect are unchanged A/B'd against a build with the city
	disabled.
	"""
	# ---- every city chunk byte-identical -----------------------------------
	var a := _detached_terrain(terrain_script, RUN_SEED)
	var b := _detached_terrain(terrain_script, SECOND_SEED)
	var all_chunks: Array[Vector2i] = _rect_chunks(a)
	var mismatched := 0
	var total_boxes := 0
	for chunk_pos: Vector2i in all_chunks:
		var sig_a := _city_signature(a, chunk_pos)
		var sig_b := _city_signature(b, chunk_pos)
		total_boxes += (sig_a[0] as Array).size()
		if var_to_bytes(sig_a) != var_to_bytes(sig_b):
			mismatched += 1
			if mismatched <= 3:
				_fail("city chunk %s differs across seeds %d vs %d (%d vs %d boxes, %d vs %d shapes)"
						% [chunk_pos, RUN_SEED, SECOND_SEED, (sig_a[0] as Array).size(), (sig_b[0] as Array).size(), (sig_a[1] as Array).size(), (sig_b[1] as Array).size()])
	if mismatched == 0:
		print("determinism: all %d city chunks byte-identical across seeds %d and %d (%d boxes total)"
				% [all_chunks.size(), RUN_SEED, SECOND_SEED, total_boxes])
	else:
		print("determinism: %d of %d city chunks mismatched across seeds" % [mismatched, all_chunks.size()])
	a.free()
	b.free()
	...
	for dx in range(-reach, reach + 1):
		for dz in range(-reach, reach + 1):
			...
			for child in parent.get_children():
				if not (child as Node).is_in_group("crocodile"):
					continue
				var name := String((child as Node).name)
