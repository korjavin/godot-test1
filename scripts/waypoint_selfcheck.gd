...
##      outside the shell's walls, on the +Z side-wall axis within 12 m of that
##      wall's face — round the corner from the door (bead godot-test1-e5i4).
##      FIVE MUTATION CONTROLS drive the same predicates with a deliberately
##      illegal point each, because "no site was wet" is also what a predicate
##      that always answers false looks like.
...
## How far off the +Z side wall's face the HQ circle may stand, and the axis it
## stands on (bead godot-test1-e5i4). It is the bead's number: the site is
## `TowerShell.OUTER_HALF + WAYPOINT_SIDE_STANDOFF` out on the side-wall axis,
## so the real gap is ~8 m and the doorway is round the corner from it.
const HQ_SIDE_MAX: float = 12.0
...
func _check_legality(terrain_script: GDScript) -> void:
	...
	THE HQ'S is outside the shell's walls, on the +Z side-wall axis within
	`HQ_SIDE_MAX` of that wall's face (bead godot-test1-e5i4) — round the corner
	from the doorway, which is cut through the +X wall, so the door is not in
	line of sight from the circle.

	FIVE MUTATION CONTROLS, each driving one of those predicates with a
	deliberately illegal point, because a predicate that always answers false
	would make every assertion above pass in silence. The fifth is the bead's
	own regression: the OLD door-axis site, which the new predicates must
	refuse.
	"""
	...
	for seed_value: int in SEEDS:
		...
		for i in sites.size():
			...
			var pos: Vector3 = sites[i]["pos"]
			if i == 0:
				var complaint := _hq_gap(pos, terrain.tower_site())
				if complaint != "":
					_fail("seed %d: %s" % [seed_value, complaint])
			elif i <= ROAD_LAST:
	...
	# THE HQ'S OWN (bead godot-test1-e5i4): the OLD door-axis site — where this
	# circle stood until this bead — must be refused by the same `_hq_gap` the
	# loop trusts. It is 48 m off the side axis, so a predicate that waved it
	# through is not measuring the corner move at all.
	var old_door: Vector3 = control.tower_site() + Vector3(TowerShell.OUTER_HALF
			+ TerrainWaypoints.WAYPOINT_SIDE_STANDOFF, 0.0, 0.0)
	if _hq_gap(old_door, control.tower_site()) == "":
		_fail("check 3's door-axis control at %s is accepted — the HQ predicates do not "
				% str(old_door) + "see the corner move, so every HQ assertion above is vacuous")
	control.free()
	...
	print("  every site of %d seeds clears the river, the rect, the discs, the hills, the "
			% SEEDS.size() + "facades and the wall; 5 mutation controls refused")
	Sentinel.done("legality")


static func _hq_gap(pos: Vector3, tower: Vector3) -> String:
	"""What is wrong with the HQ circle's site, or "" when it stands where the
	bead puts it: outside the shell's walls, on the +Z side-wall axis, within
	`HQ_SIDE_MAX` of that wall's face.

	Split out so the loop above and the door-axis control below drive the SAME
	predicates — `_plateau_gap`'s shape, and for its reason: a predicate written
	twice is a predicate that can drift.
	"""
	if absf(pos.x - tower.x) <= TowerShell.OUTER_HALF \
			and absf(pos.z - tower.z) <= TowerShell.OUTER_HALF:
		return ("the HQ circle is at %s, inside the shell's walls (±%.1f) — "
			% [str(pos), TowerShell.OUTER_HALF] + "the ring would stand in stone")
	if pos.x != tower.x:
		return ("the HQ circle is %.1f m off the +Z side-wall axis — the doorway "
			% absf(pos.x - tower.x) + "is round the corner from it, not on its line")
	var gap := pos.z - tower.z - TowerShell.OUTER_HALF
	if gap <= 0.0 or gap > HQ_SIDE_MAX:
		return ("the HQ circle is %.1f m off the +Z wall face, wanted 0 < gap <= %.1f — "
			% [gap, HQ_SIDE_MAX] + "it stands mid-wall, off the yard's edge")
	return ""
